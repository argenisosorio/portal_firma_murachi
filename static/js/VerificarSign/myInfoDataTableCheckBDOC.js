...
function InfoContentVerificarBDOC(id){
  

  for (var i = 0; i < INFO_CHECK_BDOC.signatures.length; i++) {
    if (INFO_CHECK_BDOC.signatures[i]['signatureId'] == ("S"+id)){
            ...
            html1 += '<td style="text-align: left" width="500"><span> <font style="font-size: 10pt">'+INFO_CHECK_BDOC.signatures[i]['signerCertificateSubjectName']+'</font></span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Fecha y hora de la firma:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signatureSigningTime']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Firma válida:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['isValid']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>El certificado todavía está válido:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signerCertificateIsValid']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Código postal:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+(INFO_CHECK_BDOC.signatures[i]['signaturePostalCode']?INFO_CHECK_BDOC.signatures[i]['signaturePostalCode']:"Null")+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Serial del certificado del firmante:</b> </td>';
            html1 += '<td style="text-align: left"><span>' +INFO_CHECK_BDOC.signatures[i]['signerCertificateSerial']+'</span></td>';
            html1 += '</tr>'; 
            html1 += '<tr>';
            html1 += '<td style="text-align: left" ><b>Perfil de la firma:</b> </td>';
            html1 += '<td style="text-align: left" ><span> '+INFO_CHECK_BDOC.signatures[i]['signatureProfile']+'</span></td>';
            html1 += '</tr>'; 
            html1 += '<tr>';
            html1 += '<td style="text-align: left" ><b>Estado donde se realiza la firma:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+(INFO_CHECK_BDOC.signatures[i]['signatureState']?INFO_CHECK_BDOC.signatures[i]['signatureState']:"Null")+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Algoritmo de firma utilizado:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signatureMethod']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Identificador de la firma:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signatureId']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Emisor del certificado firmante:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signerCertificateIssuer']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Ciudad donde se realiza la firma:</b> </td>';
            html1 += '<td style="text-align: left"><span> '+INFO_CHECK_BDOC.signatures[i]['signatureCity']+'</span></td>';
            html1 += '</tr>';
            html1 += '<tr>';
            html1 += '<td style="text-align: left"><b>Excepción de validación:</b> </td>';
            html1 += '<td style="text-align: left"><span>signature' +INFO_CHECK_BDOC.signatures[i]['signatureId']+'ValidationException1</span></td>';
            html1 += '</tr>'; 
            html1 += '<tr>';
            html1 += '<td style="text-align: left" ><b>País donde se realiza la firma:</b> </td>';
            html1 += '<td style="text-align: left" ><span> '+(INFO_CHECK_BDOC.signatures[i]['signatureCountry']?INFO_CHECK_BDOC.signatures[i]['signatureCountry']:"Null")+'</span></td>';
            html1 += '</tr>';
            html1 += '</table>';
            html1 += '</font>';
...
function Dialog_VerificarBDOC(id){
   document.getElementById("Dialogo_CheckBDOC").innerHTML = '\
    <style>\
       ...
       <div class="modal-dialog">\
         <div class="modal-content">\
           <div id = "title" class="modal-header"><br><br><br>\
             ...
             <h1 class="modal-title text-primary"><center>Información de la firma(S'+id+')</center></h1>\
           </div>\
    ...
    InfoContentVerificarBDOC(id);
  
}


function ColumnsDataTableBDOC(){
     var columns = [
                ...
                {title:"Información", data: function (data){
                                                ...
                                                return '<button id="informacion" type="input" class="btn btn-success btn-sm" data-toggle="modal" data-target="#myModalVerificar" onclick="javascript:Dialog_VerificarBDOC('+parseInt(conten[1])+')">Info(<font color="red">'+data.signatureId+'</font>)</button>';
                                           } 
                }
    ];
...
function DataTableVerificarBDOC(response){

    var columns = ColumnsDataTableBDOC();
    var table = $("#myCkeckBDOC").DataTable({    
